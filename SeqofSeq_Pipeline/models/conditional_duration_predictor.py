...
class ConditionalDurationPredictor(nn.Module):
    ...
    def __init__(self, config=None):
        super(ConditionalDurationPredictor, self).__init__()
        ...
        self.vocab_size = config.get('vocab_size', config['sequence_feature_dim'])
        ...
        self.token_embedding = nn.Embedding(self.vocab_size, self.d_model // 2, padding_idx=PAD_TOKEN_ID)
...
if __name__ == "__main__":
    ...
    # Initialize model
    model = ConditionalDurationPredictor(test_config)
    print(f"\n✓ Model initialized")
