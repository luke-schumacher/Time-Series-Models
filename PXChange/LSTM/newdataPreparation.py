...
file_path = 'PXChange/predicSourceIDandBodyGroup_182625.csv'  # Update with your file path
...
for sourceID in df['sourceID']:
    if sourceID == last_sourceID or last_sourceID is None:
        ...
        # Reset the counter when a new sourceID (other than 'MRI_MSR_104') is encountered
        if sourceID == 'MRI_MSR_104':
            counter = 0  # Reset to 0 for 'MRI_MSR_104'
        seq_order.append(counter)
        counter += 1
