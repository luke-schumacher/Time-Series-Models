...
from models import ConditionalDurationPredictor
...
def train_epoch(model, dataloader, optimizer, device):
    ...
    for batch in pbar:
        ...
        sequence_features = torch.zeros(batch['sequence_tokens'].shape[0], batch['sequence_tokens'].shape[1], 2).to(device)
        ...
        # Compute loss
        loss = model.compute_loss(mu, sigma, durations, mask)
        ...
        # Metrics
        mae, rmse, mape = calculate_metrics(mu, sigma, durations, mask)
...
def train_duration_model(train_loader, val_loader, config=None, training_config=None):
    """
    ...
    # Initialize model
    model = ConditionalDurationPredictor(config)
    model.to(device)
    ...
    for epoch in range(training_config['epochs']):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
                'config': config
            }, os.path.join(MODEL_SAVE_DIR, 'duration_model_best.pt'))
    ...
    # Load best model
    checkpoint = torch.load(os.path.join(MODEL_SAVE_DIR, 'duration_model_best.pt'))
    model.load_state_dict(checkpoint['model_state_dict'])

    # Plot training curves
    plot_training_curves(history, os.path.join(MODEL_SAVE_DIR, 'duration_training_curves.png'))
